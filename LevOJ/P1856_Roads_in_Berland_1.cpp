...
====关键词===================================================
1.稠密图，先floyd，得到全局最短路,之后每接收到一个新道路，判断，若该道路长度小于城市最短距离，就替换，用floyd求出最短路O(n^4)
    1点个AC，1个点WA，1个点TLE
    解决：LevOJ上要假定输入的d就是每个城市的最短路径
2.先floyd，存储每条最短路的路径，之后每接收到一个新道路，判断，若该道路长度小于城市最短距离，就替换所有用到这条路径的距离值
3.思路与2.差不多，每次接收到最短路，就
https://blog.csdn.net/h1021456873/article/details/51006290
====关键词===================================================
====题目===================================================
题目描述
...
int main()
{
    ...
    // floyd(); // floyd计算一次最短路（LevOJ上去掉这一步）
    cin >> k;
    for (int l = 1; l <= k; l++)
    {
        int r, c, dist;
        cin >> r >> c >> dist;
        if (dist < d[r][c]) //若新道路比旧最短路更短，就用新的
        {                   //题解//输出结果
            d[c][r] = d[r][c] = dist;
            //我们可以把边进行拆分，拆分成两个点，通过这两个点，不断地去更新其他的边
            //其实就是使用了局部floyd
            for (int i = 1; i <= n; i++)
            {
                for (int j = 1; j <= n; j++)
